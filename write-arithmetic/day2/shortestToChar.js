 ...
 * 返回一个整数数组 answer ，其中 answer.length == s.length 且answer[i]是s中从下标i到离它最近的字符c的距离 。
 * 两个下标 i 和 j 之间的距离为abs(i - j)，其中abs是绝对值函数。
 * 提示：
 *    1 <= s.length <= 10**4
 *    s[i] 和 c 均为小写英文字母
 ...
 * */
/**
 * 解法1. 找到C在S中的所有索引，通过对比S[i]在CArray中最小的偏移量得到结果更新res[i]。
 */
var shortestToChar = function (s, c) {
  const resArr = Array(s.length);
  const targetArr = [];
  ...
  for (let i = 0; i < s.length; i++) {
    resArr[i] = targetArr.reduce((prev, item) => {
      return Math.min(prev, Math.abs(i - item));
    }, 10001);
  }
  return resArr;
};
